...
__device__ size_t IndexToMemLoc(size_t i, CudaVec shape, CudaVec strides, size_t offset) {
  size_t idxs[MAX_VEC_SIZE];
  size_t cur_size, prev_size = 1;
  for(int j = shape.size - 1; j >= 0; j--) {
    cur_size = prev_size * shape.data[j];
    idxs[j] = i % cur_size / prev_size;
    prev_size = cur_size;
  }
  size_t loc = offset;
  for (int j = 0; j < shape.size; j++) {
    loc += idxs[j] * strides.data[j];
  }
  return loc;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  out[gid] = a[IndexToMemLoc(gid, shape, strides, offset)];
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  out[IndexToMemLoc(gid, shape, strides, offset)] = a[gid];
}
...
__global__ void ScalarSetitemKernel(scalar_t* out, scalar_t val, size_t size,  CudaVec shape,
  ...
  out[IndexToMemLoc(gid, shape, strides, offset)] = val;
}
